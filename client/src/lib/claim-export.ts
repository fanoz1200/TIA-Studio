...
export type ClaimReportPayload = {
  ...
  financialImpact?: ClaimFinancialImpact; notices?: ClaimNotice[]; review?: ClaimReviewSummary | null; generatedAt: string;
};
...
export async function buildClaimPdfBlob(payload: ClaimReportPayload) {
  ...
  claimReportSections(payload).forEach(section => { write(section.heading, 17); write(section.body, 12); }); write("سجل أحداث التأخير", 17); eventLines(payload.events).forEach(line => write(line, 11)); write("سجل الأدلة والمستندات", 17); evidenceLines(payload.evidence).forEach(line => write(line, 11)); write(`أُنشئ بواسطة TIA Studio في ${dateText(payload.generatedAt)}`, 10);

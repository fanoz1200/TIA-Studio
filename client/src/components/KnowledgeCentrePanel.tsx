...
const defaultTracks: { id: Track; title: string; description: string; outcome: string }[] = [
  { id: "tia", title: "مسار TIA العملي — Workshop 8", description: "من تثبيت الـ Baseline إلى Fragnet وقياس أثر الإكمال. الدرس التطبيقي يربط التحقق من P6 وتحديد النشاط المتأثر واعتماد Pre / Event / Post، مع بقاء ملفات التمرين الأصلية خارج التطبيق.", outcome: "نتيجة قابلة للتتبع" },
...
export function KnowledgeCentrePanel({ view, projectKey, isAuthenticated, onBeginGuidedAnalysis }: { view: string; projectKey: string; isAuthenticated: boolean; onBeginGuidedAnalysis?: () => void }) {

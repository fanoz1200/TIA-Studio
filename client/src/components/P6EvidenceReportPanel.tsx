...
import { calculateFinancialImpact, getFragnetDelayDuration, resourceAssignmentsForEvent, runCPM, type Fragnet, type Schedule, type TiaResult, type WindowTiaResult } from "@/lib/cpm";
...
import { buildP6ReconciliationManifest, serializeP6ReconciliationManifest } from "@/lib/p6-reconciliation-manifest";

type View = "guided" | "schedule" | "quality" | "event" | "analysis" | "report" | "overview" | "windows" | "methods" | "financial" | "notices" | "review" | "members" | "compare" | "resources" | "learning" | "issues";
...
export function P6EvidenceReportPanel({
  ...
  const resultQuality = useMemo(() => evaluateTiaResultQuality({ schedule, selectedEvent, analysis: activeResult }), [activeResult, schedule, selectedEvent]);
  const scheduleQuality = useMemo(() => assessScheduleQuality(schedule), [schedule]);
  const exportBlocked = workflowChecks.some((check) => check.state === "blocked") || resultQuality.state === "rejected";
  ...
  function exportWorkbook() {
    if (exportBlocked || !activeResult) { toast.error("التصدير متوقف: عالج الموانع وشغّل تحليل TIA أولاً."); return; }
    exportAnalysisExcel({ schedule, quality: scheduleQuality, analysis: activeResult, events, narrative });
    toast.success("تم إنشاء ملف Excel التحليلي متعدد الأوراق.");
  }

  function exportReconciliationManifest() {
    if (!activeResult || !("impactDays" in activeResult)) {
      toast.error("ملف المطابقة يحتاج نتيجة TIA واحدة بحدث Fragnet؛ تحليل النوافذ يظل له تقريره المنفصل.");
      return;
    }
    try {
      const manifest = buildP6ReconciliationManifest({ schedule, cpm: runCPM(schedule), tia: activeResult });
      const url = URL.createObjectURL(new Blob([serializeP6ReconciliationManifest(manifest)], { type: "application/json;charset=utf-8" }));
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = `TIA-Studio-P6-Reconciliation-${schedule.id}-${activeResult.fragnetId}.json`;
      anchor.click();
      URL.revokeObjectURL(url);
      toast.success("تم تصدير ملف المطابقة المحلي. قارنه مع P6 أو مدقق مستقل قبل وصف النتائج بأنها متطابقة.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "تعذر إنشاء ملف المطابقة المحلي.");
    }
  }

  if (view === "compare") return <ScheduleComparisonPanel currentSchedule={schedule} selectedEvent={selectedEvent} />;
  if (view === "analysis") return <TiaResultValidationPanel schedule={schedule} selectedEvent={selectedEvent} activeResult={activeResult} analystExpectedDays={analystExpectedDays} onAnalystExpectedDaysChange={setAnalystExpectedDays} />;

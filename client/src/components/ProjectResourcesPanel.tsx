...
const resources = [
  ...
  { title: "نسخة سطح المكتب — Windows 1.0.13 Setup (الموصى بها)", description: "مثبّت Windows x64 يتضمن شاشة بداية TIA Studio والشعار وبيانات الإعداد، وزر المقدمة الصوتية الاختيارية، إضافة إلى اختيار لغة الواجهة ولغة التقرير وClaim Console وتدقيق XER. يفتح شاشة البداية عند كل تشغيل، لكن لا يشغل الصوت تلقائياً؛ اضغط زر الصوت بنفسك. يضيف اختصاراً في Start ولا يحتاج Node.js أو فك ضغط. الحزمة غير موقّعة رقمياً: افحصها قبل التشغيل من هذا الرابط الرسمي فقط.", href: WINDOWS_SETUP_DOWNLOAD_URL, icon: HardDriveDownload, kind: "Windows x64 · Setup" },
  { title: "نسخة سطح المكتب — Windows 1.0.13 المحمولة", description: "ملف EXE مباشر يتضمن شاشة البداية والشعار وزر المقدمة الصوتية الاختيارية، ولا يثبت شيئاً ولا يضيف أيقونة Start. يفتح شاشة البداية عند كل تشغيل، والصوت لا يعمل إلا بعد ضغطك الصريح. مناسب للتجربة أو عند منع التثبيت في جهاز العمل.", href: WINDOWS_PORTABLE_DOWNLOAD_URL, icon: HardDriveDownload, kind: "Windows x64 · EXE محمول" },
  { title: "بصمات Windows 1.0.13 — SHA-256", description: "ملف بصمات Setup وPortable. نزّله ثم طابق البصمة قبل التشغيل، ولا تخلط ملفاً من إصدار أو مصدر تنزيل مختلف.", href: WINDOWS_SHA256_DOWNLOAD_URL, icon: ShieldCheck, kind: "SHA-256" },
...
const englishResources = [
  ...
  { title: "Desktop app — Windows 1.0.13 Setup (recommended)", description: "The Windows x64 installer includes the TIA Studio opening screen, logo and attribution, and an optional opening-audio control, plus interface/report language choices, Claim Console, and XER checks. It shows the opening screen each launch but never autoplays sound; choose the audio button yourself. It adds a Start shortcut and needs neither Node.js nor extraction. The package is not digitally signed: verify it before running and download only from this official link.", kind: "Windows x64 · Setup" },
  { title: "Desktop app — Windows 1.0.13 Portable", description: "A direct EXE with the opening screen, logo, and optional opening-audio control. It does not install anything or add a Start icon. The opening screen appears at each launch and sound starts only after your explicit click. Use it for trial work or where installation is blocked.", kind: "Windows x64 · Portable EXE" },
  { title: "Windows 1.0.13 checksums — SHA-256", description: "Checksums for Setup and Portable. Download the file and compare the checksum before running; do not mix files from different releases or download sources.", kind: "SHA-256" },
...
const resourceCopy = {
  ar: {
    ...
    setup: "Windows 1.0.13 — Setup",
    portable: "Windows 1.0.13 — محمول",
  ...
  en: {
    ...
    setup: "Windows 1.0.13 — Setup",
    portable: "Windows 1.0.13 — Portable",
    releaseEyebrow: "LIVE RELEASE GUIDE",

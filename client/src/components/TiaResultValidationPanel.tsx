...
export function TiaResultValidationPanel({ schedule, selectedEvent, activeResult, analystExpectedDays, onAnalystExpectedDaysChange }: Props) {
  ...
  return <section className="workflow-quality-gate result-validation-gate" dir={direction} aria-label={txt("لوحة تحقق نتيجة TIA", "TIA result validation")}>
    <header><div><p className="eyebrow">{txt("تحقق نتيجة TIA", "TIA RESULT VALIDATION")}</p><h3>{txt("لوحة تحقق النتيجة وقواعد القرار", "Result validation and decision rules")}</h3></div><span className={`result-validation-decision ${workflowClass}`}>{quality.state === "accepted" ? <ShieldCheck size={16} /> : <AlertTriangle size={16} />}{decision}</span></header>

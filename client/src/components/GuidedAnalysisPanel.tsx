...
export function GuidedAnalysisPanel({ schedule, xerSummary, journeyPath, journeyStep, p6GateApproved, qualityGateApproved, isXerImporting = false, baselineSnapshot, updateSnapshots, onJourneyPathChange, onJourneyStepChange, onP6GateApprovedChange, onQualityGateApprovedChange, onScheduleUpload, onNavigate, onApplyIssueExcel, onPrepareSplit, initialMethod = "TIA" }: Props) {
  ...
  const qualityChecks = useMemo(() => [
    { label: "الأنشطة المقروءة", value: schedule.activities.length, pass: schedule.activities.length > 0 },
    { label: "العلاقات المنطقية", value: schedule.relationships.length, pass: schedule.relationships.length > 0 },
    { label: "تاريخ البيانات", value: schedule.dataDate ?? "غير مسجل", pass: Boolean(schedule.dataDate) },
    { label: "التقويم والإجازات", value: schedule.calendar?.name ?? "غير مسجل", pass: Boolean(schedule.calendar?.workingWeekdays.length) },
  ], [schedule]);
  useEffect(() => {
  ...
  const nextRequirement = useMemo(() => {
    ...
    if (journeyStep === 4 && !qualityGateApproved) return { toast: "مرّر بوابة الجودة بعد مراجعة Baseline وUpdate قبل إدخال الواقعة.", detail: "راجع عدادات الجدول والتقويم وData Date، ثم أقرّ بأنك راجعت الملاحظات قبل إضافة الحدث." };
    ...
    return null;
  }, [journeyStep, journeyPath, baselineSnapshot, preEventUpdate, p6Source, p6GateApproved, qualityGateApproved, rows.length, splitPreview]);

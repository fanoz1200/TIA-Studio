...
export function ClaimContinuityPanel({ view, schedule, events, selectedWindow, isAuthenticated, onActiveClaimChange }: { view: View; schedule: Schedule; events: Fragnet[]; selectedWindow: AnalysisWindow | null; isAuthenticated: boolean; onActiveClaimChange: (claimKey: string, narrative: string) => void }) {
  ...
  const responsibilityLabels: Record<Responsibility, string> = {
    undetermined: bi("غير محددة", "Undetermined"),
    employer: bi("صاحب العمل", "Employer"),
    contractor: bi("المقاول", "Contractor"),
    neutral: bi("محايدة", "Neutral"),
    mixed: bi("مختلطة", "Mixed"),
  };
  const treatmentLabels: Record<Treatment, string> = {
    unresolved: bi("قيد التقييم", "Under assessment"),
    separate: bi("فصل الأثر", "Separate impact"),
    absorbed: bi("امتصاص ضمن المسار", "Absorbed in path"),
    apportioned: bi("توزيع للأثر", "Apportioned impact"),
  };
  ...
  return <section className="claim-continuity-panel" dir={direction}>
    <div className="claim-continuity-heading"><div><p className="eyebrow">{bi("سلسلة المطالبات · سجل التزامن", "CONTINUOUS CLAIMS · CONCURRENCY LOG")}</p><h2>{bi("سلسلة المطالبات والتأخيرات المتزامنة", "Continuous claims & concurrent delays")}</h2><p>{copy.description}</p></div><GitBranch size={24} /></div>
    {!isAuthenticated ? <div className="claim-continuity-login"><LogIn size={18} /><span>{copy.login}</span><Button className="run-button" onClick={startLogin}>{copy.signIn}</Button></div> : <>
      <div className="claim-continuity-grid"><div className="claim-chain-list"><div className="claim-chain-list__title"><b>{bi("سجل السلسلة", "Chain record")}</b><span>{copy.claims(chains.length)}</span></div>{continuity.isLoading ? <p>{copy.loading}</p> : chains.length ? chains.map(item => <button type="button" key={item.id} className={active?.id === item.id ? "claim-chain-card active" : "claim-chain-card"} onClick={() => setActiveId(item.id)}><span dir="ltr">#{item.claimKey}</span><b>{item.title}</b><small>{item.periodStart ? formatDate(item.periodStart) : copy.noStart} — {item.periodEnd ? formatDate(item.periodEnd) : copy.open}</small>{item.parentClaimId ? <em>{copy.extension}</em> : <em>{copy.chainStart}</em>}</button>) : <p className="claim-empty">{copy.noClaims}</p>}</div>
        <div className="claim-create-form"><div className="claim-chain-list__title"><b>{bi("مطالبة جديدة", "New claim")}</b><span>{copy.linksLast}</span></div><div><Label>{bi("عنوان المطالبة", "Claim title")}</Label><Input value={title} onChange={event => { setTitle(event.target.value); if (!claimKey) setClaimKey(`CLM-${keyPart(event.target.value).toUpperCase()}`); }} placeholder={copy.claimTitlePlaceholder} /></div><div><Label>{bi("مفتاح المطالبة", "Claim key")}</Label><Input dir="ltr" value={claimKey} onChange={event => setClaimKey(keyPart(event.target.value).toUpperCase())} placeholder="CLM-APR-2026" /></div><div className="claim-two"><div><Label>{bi("بداية الفترة", "Period start")}</Label><Input type="date" dir="ltr" value={from} onChange={event => setFrom(event.target.value)} /></div><div><Label>{bi("نهاية الفترة", "Period end")}</Label><Input type="date" dir="ltr" value={to} onChange={event => setTo(event.target.value)} /></div></div><div><Label>{bi("موقف فني مبدئي", "Preliminary technical position")}</Label><Textarea rows={2} value={position} onChange={event => setPosition(event.target.value)} placeholder={copy.positionPlaceholder} /></div><Button className="run-button" disabled={create.isPending || !title.trim() || !claimKey} onClick={() => create.mutate({ projectKey: schedule.id, claimKey, title: title.trim(), periodStart: from || null, periodEnd: to || null, analystPosition: position || undefined })}><PlusCircle size={16} />{bi("إنشاء مطالبة متتابعة", "Create consecutive claim")}</Button></div></div>
      {active ? <><div className="claim-active-summary"><ShieldCheck size={18} /><div><b>{bi("المطالبة النشطة", "Active claim")}: <span dir="ltr">{active.claimKey}</span> — {active.title}</b><span>{copy.activeDescription}</span></div></div><div className="claim-narrative-card"><div className="claim-chain-list__title"><b>{bi("السرد الموحد ونموذج إشعار المطالبة", "Unified narrative & Notice of Claim template")}</b><span>{bi("يُحدّث قبل التصدير", "Updated before export")}</span></div><Textarea className="claim-narrative-text" rows={14} value={active.unifiedNarrative} readOnly /><Button variant="outline" disabled={saveNarrative.isPending} onClick={() => saveNarrative.mutate({ id: active.id, analystPosition: position || undefined, status: "under_review" })}><Save size={16} />{bi("تحديث السرد وإحالته للمراجعة", "Update narrative and send it for review")}</Button></div><div className="concurrency-card"><div className="claim-continuity-heading compact"><div><p className="eyebrow">{bi("التأخيرات المتزامنة", "CONCURRENT DELAYS")}</p><h3>{bi("توثيق التزامن ضمن هذه المطالبة", "Record concurrency within this claim")}</h3></div><GitCompareArrows size={21} /></div><p className="analysis-window-reference">{bi("نافذة TIA المرجعية", "Reference TIA window")}: <b>{selectedWindow ? `${selectedWindow.id} — ${selectedWindow.name}` : bi("لم تُحدد نافذة تحليل", "No analysis window selected")}</b></p><div className="concurrency-form"><div><Label>{bi("الحدث الأول", "Primary event")}</Label><Select value={primaryEvent || "none"} onValueChange={value => setPrimaryEvent(value === "none" ? "" : value)}><SelectTrigger><SelectValue placeholder={bi("اختر الحدث", "Select event")} /></SelectTrigger><SelectContent><SelectItem value="none">{bi("اختر الحدث", "Select event")}</SelectItem>{events.map(event => <SelectItem key={event.id} value={event.id}>{event.id} — {event.title}</SelectItem>)}</SelectContent></Select></div><div><Label>{bi("الحدث المتزامن", "Concurrent event")}</Label><Select value={concurrentEvent || "none"} onValueChange={value => setConcurrentEvent(value === "none" ? "" : value)}><SelectTrigger><SelectValue placeholder={bi("اختر الحدث", "Select event")} /></SelectTrigger><SelectContent><SelectItem value="none">{bi("اختر الحدث", "Select event")}</SelectItem>{events.map(event => <SelectItem key={event.id} value={event.id}>{event.id} — {event.title}</SelectItem>)}</SelectContent></Select></div><div><Label>{bi("بداية التداخل", "Overlap start")}</Label><Input type="date" dir="ltr" value={overlapStart} onChange={event => setOverlapStart(event.target.value)} /></div><div><Label>{bi("نهاية التداخل", "Overlap end")}</Label><Input type="date" dir="ltr" value={overlapEnd} onChange={event => setOverlapEnd(event.target.value)} /></div><div><Label>{bi("المسؤولية الظاهرة", "Apparent responsibility")}</Label><Select value={responsibility} onValueChange={value => setResponsibility(value as Responsibility)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(Object.keys(responsibilityLabels) as Responsibility[]).map(value => <SelectItem key={value} value={value}>{responsibilityLabels[value]}</SelectItem>)}</SelectContent></Select></div><div><Label>{bi("معالجة التحليل", "Analysis treatment")}</Label><Select value={treatment} onValueChange={value => setTreatment(value as Treatment)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(Object.keys(treatmentLabels) as Treatment[]).map(value => <SelectItem key={value} value={value}>{treatmentLabels[value]}</SelectItem>)}</SelectContent></Select></div><div className="concurrency-wide"><Label>{bi("سبب فني وحدود التزامن", "Technical basis and concurrency limits")}</Label><Textarea rows={2} value={notes} onChange={event => setNotes(event.target.value)} placeholder={copy.notesPlaceholder} /></div></div><div className="workflow-footer"><p><AlertTriangle size={15} /> {copy.notice}</p><Button className="run-button" disabled={addConcurrency.isPending || !selectedWindow || !primaryEvent || !concurrentEvent || !overlapStart || !overlapEnd || notes.trim().length < 5} onClick={() => selectedWindow && addConcurrency.mutate({ projectKey: schedule.id, claimChainId: active.id, analysisWindowKey: selectedWindow.id, primaryEventKey: primaryEvent, concurrentEventKey: concurrentEvent, overlapStart, overlapEnd, responsibility, treatment, notes: notes.trim() })}>{bi("حفظ سجل التزامن", "Save concurrency record")}</Button></div>{activeConcurrency.length ? <div className="concurrency-records">{activeConcurrency.map(record => <div key={record.id}><b dir="ltr">{record.primaryEventKey} × {record.concurrentEventKey}</b><span>{bi("نافذة TIA المرجعية", "Reference TIA window")}: <span dir="ltr">{record.analysisWindowKey}</span> · {formatDate(record.overlapStart)} — {formatDate(record.overlapEnd)}</span><small>{responsibilityLabels[record.responsibility as Responsibility] ?? record.responsibility} · {treatmentLabels[record.treatment as Treatment] ?? record.treatment} · {record.notes}</small></div>)}</div> : <p className="claim-empty">{copy.noConcurrency}</p>}</div></> : null}</>}
  </section>;
}

...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.9 Setup (الموصى بها)").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.9/TIA-Studio-1.0.9-Windows-x64-Setup.exe");
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.9 المحمولة").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.9/TIA-Studio-1.0.9-Windows-x64-Portable.exe");
    expect(screen.getByText("بصمات Windows 1.0.9 — SHA-256").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.9/TIA-Studio-1.0.9-Windows-x64-SHA256SUMS.txt");
    ...
    expect(screen.getByRole("link", { name: /Windows 1.0.9 — Setup/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.9/TIA-Studio-1.0.9-Windows-x64-Setup.exe");
    expect(screen.getByRole("link", { name: /Windows 1.0.9 — محمول/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.9/TIA-Studio-1.0.9-Windows-x64-Portable.exe");
    expect(screen.getByRole("heading", { name: "الدليل الحيّ: كيف يعمل البرنامج من البداية للنهاية؟" })).toBeTruthy();
    expect(screen.getByText("الإصدار 1.0.9")).toBeTruthy();

...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    expect(screen.getByText("حزمة المصدر والاستمرارية — 1.0.4").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.4-Source-final.tar_6ca01026.gz");
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.4").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.4-Windows-x64-Complete_2c7d9ad8.exe");
    expect(screen.getByText("نسخة سطح المكتب — Linux 1.0.4").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.4-Linux-x64-Complete_dc3ac3f8.AppImage");
    expect(screen.getByRole("link", { name: /Windows 1.0.4 — تنزيل ملف EXE/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.4-Windows-x64-Complete_2c7d9ad8.exe");
    expect(screen.getByRole("link", { name: /Linux 1.0.4 — تنزيل AppImage/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.4-Linux-x64-Complete_dc3ac3f8.AppImage");

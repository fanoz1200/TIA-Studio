...
describe("ProjectResourcesPanel", () => {
  ...
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    const setupResourceLink = screen.getByText("نسخة سطح المكتب — Windows 1.0.13 Setup (الموصى بها) · Desktop app — Windows 1.0.13 Setup (recommended)").closest("article")?.querySelector("a");
    expect(setupResourceLink?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Setup.exe");
    expect(setupResourceLink?.getAttribute("target")).toBeNull();
    expect(setupResourceLink?.getAttribute("rel")).toBeNull();
    expect(setupResourceLink?.getAttribute("download")).toBeNull();
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.13 المحمولة · Desktop app — Windows 1.0.13 Portable").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Portable.exe");
    expect(screen.getByText(/بصمات Windows 1.0.13 — SHA-256/).closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-SHA256SUMS.txt");
    ...
    expect(fallbackSetupLink.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Setup.exe");
    expect(fallbackSetupLink.getAttribute("target")).toBeNull();
    expect(fallbackSetupLink.getAttribute("rel")).toBeNull();
    expect(screen.getByRole("link", { name: /Windows 1.0.13 — Setup/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Setup.exe");
    expect(screen.getByRole("link", { name: /Windows 1.0.13 — محمول/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Portable.exe");
    ...
    expect(screen.getByText(/إصدار Windows 1.0.13 وروابطه يظهران منفصلين/)).toBeTruthy();
    expect(screen.getByText("الإصدار · Release 1.0.13")).toBeTruthy();
  ...
  it("يعرض نصوص مركز الموارد بالإنجليزية واتجاه LTR مع حفظ روابط الإصدار وبيانات التدريب", () => {
    ...
    expect(screen.getByText(/Windows 1.0.13 and its direct links are shown separately/)).toBeTruthy();
    ...
    expect(screen.getByText(/Version 1.0.13: the TIA Studio opening screen/)).toBeTruthy();
    expect(screen.getByText("Desktop app — Windows 1.0.13 Setup (recommended)").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Setup.exe");
    expect(screen.getByRole("link", { name: "Windows 1.0.13 — Portable" }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.13/TIA-Studio-1.0.13-Windows-x64-Portable.exe");
    expect(screen.getByRole("heading", { name: "Workshop NO8 — مرجع TIA مرفوع من المستخدم" })).toBeTruthy();

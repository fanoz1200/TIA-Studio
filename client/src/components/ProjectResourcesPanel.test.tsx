...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    expect(screen.getByText("حزمة المصدر والاستمرارية — 1.0.3").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.3-Source-and-Handoff_2492ace5.zip");
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.3").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.3-Windows-x64_b812f3a0.exe");
    expect(screen.getByText("نسخة سطح المكتب — Linux 1.0.3").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.3-Linux-x64_9684b12c.AppImage");
    expect(screen.getByRole("link", { name: /Windows 1.0.3 — تنزيل ملف EXE/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.3-Windows-x64_b812f3a0.exe");
    expect(screen.getByRole("link", { name: /Linux 1.0.3 — تنزيل AppImage/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.3-Linux-x64_9684b12c.AppImage");
    expect(screen.getByRole("heading", { name: "الدليل الحيّ: كيف يعمل البرنامج من البداية للنهاية؟" })).toBeTruthy();
    expect(screen.getByText("الإصدار 1.0.3")).toBeTruthy();

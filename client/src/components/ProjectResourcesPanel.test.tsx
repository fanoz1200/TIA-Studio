...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    const setupResourceLink = screen.getByText("نسخة سطح المكتب — Windows 1.0.11 Setup (الموصى بها)").closest("article")?.querySelector("a");
    expect(setupResourceLink?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-Setup.exe");
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.11 المحمولة").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-Portable.exe");
    expect(screen.getByText("بصمات Windows 1.0.11 — SHA-256").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-SHA256SUMS.txt");
    ...
    expect(fallbackSetupLink.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-Setup.exe");
    expect(fallbackSetupLink.getAttribute("target")).toBe("_blank");
    expect(fallbackSetupLink.getAttribute("rel")).toBe("noreferrer");
    expect(screen.getByRole("link", { name: /Windows 1.0.11 — Setup/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-Setup.exe");
    expect(screen.getByRole("link", { name: /Windows 1.0.11 — محمول/ }).getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.11/TIA-Studio-1.0.11-Windows-x64-Portable.exe");
    expect(screen.getByRole("heading", { name: "الدليل الحيّ: كيف يعمل البرنامج من البداية للنهاية؟" })).toBeTruthy();
    expect(screen.getByText("الإصدار 1.0.11")).toBeTruthy();

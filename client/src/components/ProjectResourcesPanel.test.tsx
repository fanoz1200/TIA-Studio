...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    expect(screen.getByText("حزمة المصدر والاستمرارية — 1.0.5 (Workshop)").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.5-source.tar_433b5530.gz");
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.5 (Workshop)").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.5-Windows-x64_290fa539.exe");
    expect(screen.getByText("نسخة سطح المكتب — Linux 1.0.5 (Workshop)").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.5-Linux-x64_1ec045c9.AppImage");
    expect(screen.getByRole("link", { name: /Windows 1.0.5 \(Workshop\) — تنزيل EXE/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.5-Windows-x64_290fa539.exe");
    expect(screen.getByRole("link", { name: /Linux 1.0.5 \(Workshop\) — تنزيل AppImage/ }).getAttribute("href")).toBe("/manus-storage/TIA-Studio-1.0.5-Linux-x64_1ec045c9.AppImage");
    expect(screen.getByRole("heading", { name: "الدليل الحيّ: كيف يعمل البرنامج من البداية للنهاية؟" })).toBeTruthy();
    expect(screen.getByText("الإصدار 1.0.5")).toBeTruthy();

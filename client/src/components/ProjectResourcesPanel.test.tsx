...
describe("ProjectResourcesPanel", () => {
  it("يعرض الأدلة والأمثلة القابلة للتنزيل وحدود المعالجة المحلية", () => {
    ...
    const setupResourceLink = screen.getByText("نسخة سطح المكتب — Windows 1.0.12 Setup (الموصى بها)").closest("article")?.querySelector("a");
    ...
    expect(screen.getByText("نسخة سطح المكتب — Windows 1.0.12 المحمولة").closest("article")?.querySelector("a")?.getAttribute("href")).toBe("https://github.com/fanoz1200/TIA-Studio/releases/download/v1.0.12/TIA-Studio-1.0.12-Windows-x64-Portable.exe");
    ...
    expect(screen.getByText("الإصدار 1.0.12")).toBeTruthy();

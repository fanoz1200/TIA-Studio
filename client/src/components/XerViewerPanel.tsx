...
function ScheduleFacts({ schedule, summary, language }: { schedule: Schedule; summary?: XerImportSummary; language: "ar" | "en" }) {
  ...
  return (
    <dl className="xer-viewer__facts">
      <div><dt>Data Date</dt><dd>{schedule.dataDate ?? txt("غير مسجل", "Not recorded")}</dd></div>
      <div><dt>{txt("الأنشطة", "Activities")}</dt><dd>{schedule.activities.length}</dd></div>
      <div><dt>{txt("العلاقات", "Relationships")}</dt><dd>{schedule.relationships.length}</dd></div>
      <div><dt>WBS</dt><dd>{new Set(schedule.activities.map(item => item.wbs).filter(Boolean)).size || txt("غير مقروء", "Not read")}</dd></div>
      <div><dt>{txt("التقويم", "Calendar")}</dt><dd>{calendar?.name ?? txt("غير مسجل", "Not recorded")}</dd></div>
      <div><dt>{txt("أيام العمل", "Working days")}</dt><dd>{calendar?.workingWeekdays?.length ?? 0} {txt("أيام/أسبوع", "days/week")}</dd></div>
      <div><dt>{txt("تقويمات الأنشطة", "Activity calendars")}</dt><dd>{summary ? `${summary.taskCalendarCount ?? summary.taskCalendarIds?.length ?? 0} ${txt("معرّف", "ID(s)")}` : txt("غير مقروء", "Not read")}</dd></div>
      <div><dt>{txt("قيود XER", "XER constraints")}</dt><dd>{summary ? `${summary.supportedConstraintsRead ?? 0} ${txt("محسوب", "supported")} · ${summary.unsupportedConstraintsRead ?? 0} ${txt("مراجعة", "review")}` : txt("غير مقروء", "Not read")}</dd></div>
      <div><dt>{txt("حالة التحديث", "Update state")}</dt><dd>{summary ? `${summary.activitiesWithProgress ?? 0} ${txt("نشاط — محفوظة للمراجعة، لا F9 محلي", "activity(ies) — retained for review; no local F9")}` : txt("غير مقروء", "Not read")}</dd></div>
    </dl>
  );
}
...
export function XerViewerPanel({ schedule, xerSummary, baselineSnapshot, updateSnapshots, onNavigate }: Props) {
  ...
  return <section className="xer-viewer" aria-labelledby="xer-viewer-title" dir={direction}>
    <header className="xer-viewer__header">
      <div className="xer-viewer__title"><span><FileCode2 size={22} /></span><div><p className="eyebrow">{txt("مراجعة محلية قبل Primavera", "Local review before Primavera")}</p><h1 id="xer-viewer-title">{txt("عارض XER والنسخ", "XER and snapshot viewer")}</h1><p>{txt("تقدر تشوف اللي البرنامج قرأه من الملف، وتراجع الفرق قبل ما تعمل Reverse Import أو F9 على نسخة Primavera غير إنتاجية.", "Review what the application read from the file and compare snapshots before Reverse Import or F9 in a non-production Primavera copy.")}</p></div></div>
      <div className="xer-viewer__actions"><Button variant="outline" onClick={() => onNavigate("schedule")}>{txt("ارفع أو استبدل ملف", "Upload or replace file")}</Button><Button onClick={() => onNavigate("compare")}><GitCompareArrows size={16} /> {txt("افتح المقارنة", "Open comparison")}</Button></div>
    </header>

    <div className="xer-viewer__notice"><AlertTriangle size={18} /><p><b>{txt("حدود العارض:", "Viewer limits:")}</b> {txt("قراءة ومراجعة محلية فقط. لا يعدّل ملف XER الأصلي، ولا يثبت تطابقاً كاملاً مع Primavera أو صلاحية أي مطالبة.", "Local reading and review only. It does not modify the original XER, and it does not prove full Primavera equivalence or entitlement for any claim.")}</p></div>

    <div className="xer-viewer__summary">
      <article><Network size={20} /><div><span>{txt("المشروع المفتوح", "Open project")}</span><strong>{schedule.name}</strong><small>{txt("المصدر:", "Source:")} {schedule.source ?? txt("محلي", "Local")}</small></div></article>
      <article><CalendarDays size={20} /><div><span>{txt("التقويم المستخدم", "Calendar in use")}</span><strong>{schedule.calendar?.name ?? txt("غير مسجل", "Not recorded")}</strong><small>{schedule.calendar?.holidaySource ?? txt("راجع التقويم يدوياً", "Review the calendar manually")}</small></div></article>
      <article><FileCode2 size={20} /><div><span>{txt("آخر قراءة XER", "Latest XER read")}</span><strong>{xerSummary ? `${xerSummary.activitiesRead} ${txt("نشاط", "activity(ies)")}` : txt("لم يُرفع XER بعد", "No XER uploaded yet")}</strong><small>{xerSummary ? `${xerSummary.relationshipsRead} ${txt("علاقة مقروءة", "relationship(s) read")}` : txt("يمكنك البدء بملف XER أو P6 XML", "Start with an XER or P6 XML file")}</small></div></article>
    </div>

    <section className="xer-viewer__section" aria-labelledby="xer-files-title"><div className="xer-viewer__section-heading"><div><h2 id="xer-files-title">{txt("الملفات والنسخ اللي بتقارنها", "Files and snapshots being compared")}</h2><p>{txt("كل ملف مرفوع يفضل منفصل؛ البرنامج لا يخلط Baseline مع الـUpdates تلقائياً.", "Each uploaded file remains separate; the application does not automatically merge the baseline with updates.")}</p></div><Badge className="badge-muted">{snapshots.length} {txt("ملف/نسخة", "file(s)/snapshot(s)")}</Badge></div>
    ...
    <section className="xer-viewer__section" aria-labelledby="xer-diff-title"><div className="xer-viewer__section-heading"><div><h2 id="xer-diff-title">{txt("فرق سريع بين Baseline وآخر نسخة", "Quick baseline-to-current comparison")}</h2><p>{txt("دي مقارنة تعريفية بالمعرفات والعدادات، مش حكم تأخيري نهائي.", "This is an identifier-and-count comparison, not a final delay finding.")}</p></div></div>
      <div className="xer-viewer__diff"><article><span>{txt("أنشطة أُضيفت", "Activities added")}</span><strong>{addedActivities.length}</strong><p>{addedActivities.slice(0, 5).map(item => item.id).join("، ") || txt("لا يوجد", "None")}</p></article><article><span>{txt("أنشطة اختفت", "Activities removed")}</span><strong>{removedActivities.length}</strong><p>{removedActivities.slice(0, 5).map(item => item.id).join("، ") || txt("لا يوجد", "None")}</p></article><article><span>{txt("فرق العلاقات", "Relationship difference")}</span><strong>{current.relationships.length - baseline.relationships.length > 0 ? "+" : ""}{current.relationships.length - baseline.relationships.length}</strong><p>{language === "en" ? `${baseline.relationships.length} in baseline → ${current.relationships.length} in current snapshot` : `${baseline.relationships.length} في Baseline ← ${current.relationships.length} في النسخة الحالية`}</p></article><article><span>{txt("فرق Data Date", "Data Date difference")}</span><strong>{baseline.dataDate === current.dataDate ? txt("نفس التاريخ", "Same date") : txt("راجع", "Review")}</strong><p>{baseline.dataDate ?? txt("غير مسجل", "Not recorded")} ← {current.dataDate ?? txt("غير مسجل", "Not recorded")}</p></article></div></section>

    <section className="xer-viewer__section" aria-labelledby="xer-activities-title"><div className="xer-viewer__section-heading"><div><h2 id="xer-activities-title">{txt("معاينة الأنشطة المقروءة", "Read activity preview")}</h2><p>{txt("أول 30 نشاطاً من البرنامج المفتوح حالياً. استخدم جدول البرنامج للبحث والفلترة الكاملة.", "The first 30 activities from the current schedule. Use the schedule table for full search and filtering.")}</p></div></div><div className="xer-viewer__table-wrap"><table><thead><tr><th>Activity ID</th><th>{txt("اسم النشاط", "Activity name")}</th><th>WBS</th><th>{txt("المدة", "Duration")}</th><th>{txt("المسؤول", "Owner")}</th></tr></thead><tbody>{schedule.activities.slice(0, 30).map(activity => <tr key={activity.id}><td dir="ltr">{activity.id}</td><td>{activity.name}</td><td>{activity.wbs ?? "—"}</td><td>{activity.duration} {txt("يوم", "day(s)")}</td><td>{activity.owner ?? "—"}</td></tr>)}</tbody></table></div></section>
  </section>;
}

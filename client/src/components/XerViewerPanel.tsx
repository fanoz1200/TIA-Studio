...
export function XerViewerPanel({ schedule, xerSummary, baselineSnapshot, updateSnapshots, onNavigate }: Props) {
  ...
  return <section className="xer-viewer" aria-labelledby="xer-viewer-title" dir={direction}>
    ...
    <div className="xer-viewer__summary">
      ...
      <article><CalendarDays size={20} /><div><span>{ui("التقويم المستخدم", "Calendar in use")}</span><strong>{schedule.calendar?.name ?? txt("غير مسجل", "Not recorded")}</strong><small>{schedule.calendar?.holidaySource ?? txt("راجع التقويم يدوياً", "Review the calendar manually")}</small></div></article>

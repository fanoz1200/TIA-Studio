...
import { AlertTriangle, Archive, ArrowLeftRight, CheckCircle2, Download, FileDiff, FileUp, LoaderCircle, Plus, Trash2 } from "lucide-react";
...
export function ScheduleComparisonPanel({ currentSchedule, selectedEvent, events = [] }: { currentSchedule: Schedule; selectedEvent?: Fragnet | null; events?: Fragnet[] }) {
  ...
  const tx = language === "en" ? {
    ...
    loadSuccess: "تم تحميل", openSuccess: "حُمّل البرنامج المفتوح كبرنامج مرجعي.", genericReadError: "تعذر قراءة ملف البرنامج.", xerQualityBlocked: "أُوقف تنزيل XER: صحح موانع جودة البرنامج أولاً", xerRoundTripBlocked: "أُوقف تنزيل XER:", xerRoundTripFallback: "فشل فحص الاستيراد العكسي.", xerDownloadSuccess: "تم تنزيل", preTiaSnapshot: "قبل TIA", postTiaSnapshot: "بعد TIA", xerDownloaded: "التجريبي بعد فحص الاستيراد العكسي:", relationships: "علاقة", conjunction: "و", xerCreateError: "تعذر إنشاء ملف XER التجريبي.", xerReview: "راجِع قيود التقويم وحقول P6 غير المدعومة في رسالة التنزيل قبل استخدام الملف خارجياً.", selectEvent: bi("اختر حدثاً أولاً.", "Select an event first."), preservedBlocked: bi("أُوقف تصدير XER المحافظ:", "Preserved XER export stopped:"), preservedDownloaded: bi("تم التنزيل من ملف XER الأصلي المحفوظ محلياً.", "Downloaded from the original local XER."), packageReady: bi("تم إنشاء حزمة الأحداث محلياً.", "Local event package created."), packageBlocked: bi("تعذر إنشاء حزمة الأحداث.", "Event package could not be created."), p6Review: bi("لا تعتبر هذا تطابقاً مع P6: افتح Post في نسخة P6 مستقلة، نفذ Schedule/F9، ثم قارن التواريخ والـ Float والأعداد.", "Do not treat this as a P6 parity result: import Post to a separate P6 copy, Schedule/F9, then compare dates, float, and counts."), sourceMissing: bi("ملف XER الأصلي غير محفوظ في جلسة المتصفح الحالية. أعد استيراد XER الأصلي لاستخدام Pre/Post المحافظ.", "No original XER is held in this browser session. Re-import the original XER to use preserved Pre/Post."), calendarReview: bi("مراجعة مرجع التقويم", "Calendar reference review"),
  };
  ...
  function downloadExperimentalXer(snapshot: "pre-tia" | "post-tia") {
    try {
  ...
  async function exportEventPackage() {
    const packageEvents = events.length ? events : selectedEvent ? [selectedEvent] : [];
    if (!packageEvents.length) { toast.error(tx.selectEvent); return; }
    setIsPackaging(true);
    try {
      const output = await buildPreservedEventPackageZip(currentSchedule, packageEvents);
      downloadBlob(output.fileName, output.blob);
      toast.success(tx.packageReady);
      output.messages.forEach((message) => toast.warning(message));
      toast.info(tx.p6Review);
    } catch (error) { toast.error(error instanceof Error ? error.message : tx.packageBlocked); }
  ...
  const calendarAssessment = currentSchedule.xerSource ? assessPrimaveraCalendarMatch(currentSchedule) : null;
  return <div className="view-stack comparison-view" dir={direction}><section className="page-heading"><div><p className="eyebrow">{tx.eyebrow}</p><h1>{tx.title}</h1><p>{tx.description}</p><p className="context-tip">{tx.context}</p></div><div className="heading-actions comparison-heading-actions"><Button variant="outline" className="outline-action" onClick={() => { setBaseline(currentSchedule); toast.success(tx.openSuccess); }}><Plus size={16} />{tx.useOpen}</Button>{currentSchedule.xerSource ? <><Button variant="outline" className="outline-action" disabled={!selectedEvent} onClick={() => downloadPreserved("pre")}><Download size={16} />{tx.preXer}</Button><Button variant="outline" className="outline-action" disabled={!selectedEvent} onClick={() => downloadPreserved("post")}><Download size={16} />{tx.postXer}</Button><Button variant="outline" className="outline-action" disabled={isPackaging} onClick={exportEventPackage}>{isPackaging ? <LoaderCircle size={16} className="animate-spin" /> : <Archive size={16} />}{tx.eventPackage}</Button></> : <Button variant="outline" className="outline-action" onClick={() => downloadExperimentalXer("pre-tia")}><Download size={16} />{tx.experimentalXer}</Button>}</div></section>
    {calendarAssessment ? <section className="comparison-warnings">{calendarAssessment.messages.map((message, index) => <div key={`${index}-${message}`}><AlertTriangle size={17} /><span>{index === 0 ? <b>{tx.calendarReview}: </b> : null}{message}</span></div>)}</section> : null}

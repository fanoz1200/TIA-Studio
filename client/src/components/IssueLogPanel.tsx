...
import { AlertTriangle, CheckCircle2, ClipboardList, FileCog, LogIn, Plus, ShieldCheck, XCircle } from "lucide-react";
...
export function IssueLogPanel({ view, schedule, existingEvents, isAuthenticated, onApplyFragnet }: { view: View; schedule: Schedule; existingEvents: Fragnet[]; isAuthenticated: boolean; onApplyFragnet: (event: Fragnet) => void }) {
  ...
  const create = trpc.issueLog.create.useMutation({ onSuccess: () => { utils.issueLog.list.invalidate(projectInput); setTitle(""); setDescription(""); setIssueNo(""); toast.success("تم حفظ القضية ومقترح الـ Fragnet للمراجعة."); }, onError: error => toast.error(error.message) });
  ...
  const canCreate = Boolean(issueNo.trim() && title.trim() && description.trim() && occurrenceDate && relationshipId && affectedActivityIds.length && Number(duration) > 0);
  ...
  return <section className="issue-log-panel"><div className="issue-log-heading"><div><p className="eyebrow">PLANNER ISSUE LOG</p><h2>سجل القضايا المؤثر في البرنامج</h2><p>لا يُضاف أي أثر إلى شبكة TIA من هذه الشاشة إلا بعد تجهيز المقترح واعتماده يدوياً. السجل يوثق المسؤولية والحرجية والأنشطة والعلاقة المنطقية المرجعية.</p></div><ClipboardList size={24} /></div>
    ...
    <div className="issue-form">
      ...
      <div className="issue-form-wide"><Label>الوصف الفني والمراجع</Label><Textarea rows={4} value={description} onChange={event => setDescription(event.target.value)} placeholder="اشرح الوقائع، المرجع التعاقدي أو المحضر، وما الذي يجعل العلاقة أو الأنشطة المختارة مناسبة للتحليل…" /></div>
    </div>
    <div className="issue-log-actions"><p><AlertTriangle size={16} /> تحقّق من العلاقة والمدة قبل حفظ القضية؛ تعديل الأثر بعد التطبيق يكون بقضية تصحيحية جديدة لحفظ التدقيق.</p><Button className="run-button" disabled={create.isPending || !canCreate} onClick={() => create.mutate({ projectKey: schedule.id, issueNo: issueNo.trim(), title: title.trim(), description: description.trim(), occurrenceDate, reportedBy: reportedBy.trim() || undefined, responsibleParty: responsibility, delayCause: cause, affectedActivityIds, replacedRelationshipId: relationshipId, proposedDurationDays: Number(duration), criticality })}><Plus size={16} />حفظ قضية ومقترح Fragnet</Button></div>

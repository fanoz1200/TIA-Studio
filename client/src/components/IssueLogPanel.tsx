...
export function IssueLogPanel({ view, schedule, existingEvents, isAuthenticated, onApplyFragnet }: { view: View; schedule: Schedule; existingEvents: Fragnet[]; isAuthenticated: boolean; onApplyFragnet: (event: Fragnet) => void }) {
  ...
  if (!isAuthenticated) return <section className="issue-log-panel" dir={direction}><div className="issue-log-heading"><div><p className="eyebrow">{copy.plannerIssueLog}</p><h2>{copy.loginHeading}</h2><p>{copy.loginDescription}</p></div><ClipboardList size={24} /></div><div className="issue-log-login"><LogIn size={19} /><span>{copy.loginPrompt}</span><Button className="run-button" onClick={startLogin}>{copy.login}</Button></div></section>;
  ...
  return <section className="issue-log-panel" dir={direction}><div className="issue-log-heading"><div><p className="eyebrow">{copy.plannerIssueLog}</p><h2>{copy.heading}</h2><p>{copy.headingDescription}</p></div><ClipboardList size={24} /></div>
    <div className="issue-log-caution"><ShieldCheck size={17} /><span>{copy.caution}</span></div>
    <div className="issue-exchange"><div><Table2 size={20} /><div><b>{copy.excelExchange}</b><span>{copy.excelHelp}</span></div></div><div className="issue-exchange-actions"><Button variant="outline" onClick={downloadIssueImportTemplate}><Download size={16} />{copy.excelTemplate}</Button><Button variant="outline" disabled={!issues.data?.length} onClick={exportIssues}><Download size={16} />{copy.exportRegister}</Button><input ref={importInputRef} type="file" accept=".xlsx,.xls" hidden onChange={event => { handleExcel(event.target.files?.[0]); event.currentTarget.value = ""; }} /><Button variant="outline" onClick={() => importInputRef.current?.click()}><FileUp size={16} />{copy.importExcel}</Button></div></div>
    ...
    <div className="issue-form">
      <div><Label>{copy.issueNumber}</Label><Input value={issueNo} onChange={event => setIssueNo(event.target.value)} placeholder="ISS-001" /></div>
      <div><Label>{copy.issueTitle}</Label><Input value={title} onChange={event => setTitle(event.target.value)} placeholder={language === "en" ? "Late change instruction" : "تعليمات تغيير متأخرة"} /></div>
      <div><Label>{copy.occurrenceDate}</Label><Input type="date" dir="ltr" value={occurrenceDate} onChange={event => setOccurrenceDate(event.target.value)} /></div>
